...
#     exit()
    ...
    pipeline = AnimationPipeline(
        vae=vae, text_encoder=text_encoder, tokenizer=tokenizer, unet=unet, controlnet1 = controlnet1,
    ...
    test_videos1 = config.video_path1
    ...
    config.random_seed = []
    prompt = n_prompt = ""
    for idx, (source_image, test_video, test_video1, random_seed, size, step) in tqdm(
        enumerate(zip(source_images, test_videos, test_videos1, random_seeds, sizes, steps)), 
        ...
        if test_video1.endswith('.mp4'):
            control1 = VideoReader(test_video1).read()
            if control1[0].shape[0] != size:
                control1 = [np.array(Image.fromarray(c).resize((size, size))) for c in control1]
            if config.max_length is not None:
                control1 = control1[config.offset: (config.offset+config.max_length)]
        ...
        if control1.shape[0] % config.L > 0:
            control1 = np.pad(control1, ((0, config.L-control1.shape[0] % config.L), (0, 0), (0, 0), (0, 0)), mode='edge')
        ...
        sample = pipeline(
            ...
            controlnet_condition1    = control,
            controlnet_condition2    = control1,
