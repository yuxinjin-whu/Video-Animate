...
class AnimationPipeline(DiffusionPipeline):
    ...
    def prepare_condition(self, condition1, condition2, num_videos_per_prompt, device, dtype, do_classifier_free_guidance):
        ...
        if do_classifier_free_guidance:
            condition1 = torch.cat([condition1] * 2)
        ...
        if do_classifier_free_guidance:
            condition2 = torch.cat([condition2] * 2)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        control1, control2 = self.prepare_condition(
        ...
        controlnet_uncond_images1, controlnet_cond_images1 = control1.chunk(2)
        controlnet_uncond_images2, controlnet_cond_images2 = control2.chunk(2)
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps), disable=(rank!=0)):
            ...
            for i in range(num_context_batches):
                ...
                down_block_res_samples1, mid_block_res_sample1 = self.controlnet1(
                    controlnet_latent_input,
                    t,
                    encoder_hidden_states=torch.cat([controlnet_text_embeddings_c[c] for c in context]),
                    controlnet_cond=torch.cat([controlnet_cond_images1[c] for c in context]),
                ...
                down_block_res_samples2, mid_block_res_sample2 = self.controlnet2(
                    ...
                    t,
                    encoder_hidden_states=torch.cat([controlnet_text_embeddings_c[c] for c in context]),
                    controlnet_cond=torch.cat([controlnet_cond_images2[c] for c in context]),
                    conditioning_scale=controlnet_conditioning_scale,
                    return_dict=False,
                )
                down_block_res_samples = [
                    samples_prev + samples_curr
                    for samples_prev, samples_curr in zip(down_block_res_samples1, down_block_res_samples2)
                ]
